...
def test(test_dataset, pretrained_model_path, batch_size=16, num_of_workers=0, num_of_classes=1200):
    r"""Testing a Model
    Args:
        :param test_dataset: (RGBD_Dataset)
        ...
        :return: the predicted class(es) of the input dataset
    """
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    test_dataloader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False,
                                 num_workers=num_of_workers, drop_last=True)

    model = ResNet50(test_dataset.input_channels,
                     num_of_classes, pretrained=False)
    ...
    for step, inputs in enumerate(test_dataloader):
        ...
        # p_bar.set_description(
            # '[Evaluation Loss: {:.4f} Acc: {:.2f}%]'.format(loss, accu))
        p_bar.update(1)
    p_bar.close()
...
if __name__ == '__main__':
    ...
    test(test_dataset, batch_size=args.batch_size,
